    ...
    for chap_list_key in list_dict:
        if ("Chapters_" in chap_list_key):
            ...
            for ep_info in ep_arry['Episode']:
                ep_list_text = ('<li>Episode:{} <a href="{}">{}</a> /{}</li>')
                print(ep_list_text.format(ep_info['ep_num'], ep_info['url'], ep_info['title'], ep_info['publishedAt']), file=f_html)
